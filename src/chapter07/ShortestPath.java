...
public class ShortestPath {
    ...
    // 用来存放已经找到最短路径的点的集合
    Set<Integer> foundedSet = new HashSet<Integer>();
    ...
    public void find() {
        ...
        while (foundedSet.size() != matrix.length) {
            ...
            // 放入findedset
            foundedSet.add(currentMinIndex);
        }
    }
    ...
    private int currentMinIndex() {
        ...
        while (it.hasNext()) {
            Entry<Integer, Integer> entry = it.next();
            if (!foundedSet.contains(entry.getKey()) && entry.getValue() < min) {
                min = entry.getValue();
